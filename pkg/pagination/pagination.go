...
// New 分页对象构造器
// r -- 用来获取分页的 URL 蚕食，默认是 page，可通过 config/pagination.go 修改
// db -- Gorm 查询句柄，用以查询数据集和获取数据总数
// baseURL -- 用以分页链接
// PerPage -- 每页条数，传参为小于或等于 0 时为默认值 10 ，可通过 config/pagination.go 修改
...
func (p Pagination) Results(data any) error {
	var err error
	var offset int
	page := p.CurrentPage()
	if page == 0 {
	...
	return p.db.Preload(clause.Associations).
		Limit(p.PerPage).
		Offset(offset).
		Find(data).Error
}

// TotalCount 返回数据库中的条数
